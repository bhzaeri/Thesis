...
\end{figure}
\subsection{Social fabric based influence function} In this model, the individuals might decide to change their controller KS using the majority of KSs which they receive from their neighborhood. Here, Majority Voting is used to find the controller KS of an individual \citet{che2010robust}. Figures \ref{fig:SFNetworkModel} describes the social fabric influence function. In the case of a tie, some tie-breaking rules are deployed such as MFU (most frequently used), LFU (least frequently used), Direct (the direct influencing KS), Random (a random choice), Last-used (the KS which controlled the KS in the last iteration) \citet{ali2012socio}. In the equation \ref{eq:4} the sum denotes the counts of KSs in $i$th node neighborhood and $\psi_{i}$ is its contoller KS. $\tau_{i}$ is the net affecting KS for the next iteration \citet{ali2016leveraged} \citet{sterling2004aggregation}. 
\begin{equation}
\label{eq:4}
...
\begin{figure}[h]
	...
	\centering
	\caption{Social Fabric Influence Function\citet{5284844}}
...
Each particle is comprised of a position vector and velocity vector \citet{bratton2007defining}. Particles adjust their velocities and positions as follows:
\begin{equation}
